...
- The posterior distribution for mean $\mu$ ($\phi = 1/\sigma^2$ and $\phi_0 = 1/\tau^2$): 
    \begin{equation}
...
- The posterior distribution for mean $\mu$: 
    \begin{equation}
    \mu \mid y_1, \cdots, y_n, {\color{red}\phi} \sim \textrm{Normal}\left(\frac{\phi_0 \theta + n\phi\bar{y} }{\phi_0 + n\phi}, \sqrt{\frac{1}{\phi_0 + n \phi}}\right)
    \end{equation}
    where the precision $\phi = 1/\sigma^2$ and $\phi_0 = 1/\tau^2$ which are known since $\sigma^2$ and $\tau^2$ are known
...
- The posterior distribution for variance $\sigma^2$: 
    \begin{equation}
...
::: {.callout-note}
THIS SHOULD BE A DISCUSSION QUESTION CALLOUT
...
- $\mu^{(s)} \sim \pi(\mu \mid \phi^{(s-1)}, y_1, \cdots, y_n)$

- $\phi^{(s)} \sim \pi(\phi \mid \mu^{(s)}, y_1, \cdots, y_n)$
...
posterior distribution of $(\mu, \phi \mid y_1, \cdots, y_n)$
:::
...
::: {.callout-note}
THIS SHOULD BE AN EXERCISE CALLOUT
...
# Use Stan and Bayesian Inferences

# Markov Chain Monte Carlo and Diagnostics