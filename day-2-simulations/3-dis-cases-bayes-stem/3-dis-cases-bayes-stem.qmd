...
Here we see a large p-value, indicating that assuming there is no difference in tumor rates between the groups, our data, or data that are more extreme, are fairly likely.

:::

## Unusual Results

Scientists in the National Toxicology Program saw the results quite differently.  Histiocytic sarcomas are exceedingly rare. One researcher pointed out that across 6 prior studies involving 460 female rats, no animals in the control groups developed this type of tumor. 
...
We can evaluate the hypothesis that the posterior mean tumor rate in the benzophenone group is greater than that in the control group by simulating values of the proportions from the respectives beta posteriors and counting how many times we see a greater tumor proportion in the benzophenone group than among controls.

```
x1=rbeta(10000,0.1,510)
x2=rbeta(10000,4,148)
mean(x2>x1)
```
